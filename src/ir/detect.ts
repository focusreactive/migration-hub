...
export const platformHintsSchema = z.strictObject({
  ...
  framerRouteId: z.string().optional(),
  framerBreakpoints: z.array(z.strictObject({ hash: z.string(), mediaQuery: z.string() })).optional(),
});

export type PlatformHints = z.infer<typeof platformHintsSchema>;

export const detectDataSchema = z.strictObject({
  verdict: z.enum(["webflow", "framer", "unknown", "ambiguous"]),
  scores: z.strictObject({ webflow: platformScoreSchema, framer: platformScoreSchema }),
  thresholds: z.strictObject({ confidence: z.number(), margin: z.number() }),
