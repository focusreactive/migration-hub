import type { ComplexityAreaId } from "../analysis/complexity.ts";
...
function designSystemParagraph(metrics: ReportMetrics, input: ReportInput): string {
  ...
  return (
    ...
    + `${countLabel(metrics.images, "unique image", "unique images")}, with ${videoClause}. The only real task `
    + `here is re-hosting: every asset currently lives on ${platform}'s CDN and those URLs stop working when the `
    + "site is unpublished."
  );
}
...
export const COMPLEXITY_PARAGRAPHS: Record<ComplexityAreaId, (metrics: ReportMetrics, input: ReportInput) => string> = {
  contentModel: (metrics) =>
    `${countLabel(metrics.collections, "collection", "collections")}, all flat, each with a single dynamic `
    + "segment in its route. There is no nesting, no cross-referencing between collections, and no page that "
    + "belongs to two of them at once. A structure like this maps almost one-to-one onto document types in "
    + "Sanity or collections in Payload, and the route patterns themselves tell us what the slug fields and "
    + "templates need to be.",
  pageComposition: (metrics) =>
    ...
    + "deciding deliberately at the start rather than retrofitting later. This is the area that sets the pace "
    + "of the whole project.",
  designSystem: designSystemParagraph,
  forms: formsParagraph,
  contentVolume: (metrics) =>
    `${countLabel(metrics.entries, "published entry", "published entries")} across `
    + `${countLabel(metrics.collections, "collection", "collections")}. That fits into a single automated `
    + "migration pass with room to review every record by hand afterwards, and it keeps the content freeze short.",
};
