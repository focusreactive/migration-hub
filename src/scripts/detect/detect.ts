...
export async function runDetect(projectPath: string, force: boolean): Promise<void> {
  ...
  const computed = await withStep(
    ...
    async () => {
      ...
      await writeArtifact(projectPath, detectArtifact, detect);
  ...
  const detect: DetectData = wasSkipped ? await readArtifact(projectPath, detectArtifact) : (computed as DetectData);
  ...
  console.log(
    JSON.stringify({
      ...
      scores: {
        webflow: detect.scores.webflow.score,
        framer: detect.scores.framer.score,
      },
    }),
  );
}
