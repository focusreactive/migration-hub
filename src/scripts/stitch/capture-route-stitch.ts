...
import { stitchStrips, type StitchStrip } from "#lib/capture/stitch-strips.ts";
...
import { assignStickyOwners, captureStripViaCdp, withFrozenAnimations } from "./utils/capture-route-stitch.ts";
import {
...
export async function captureRouteStitch(page: Page, cdp: CDPSession, viewport: CaptureViewport): Promise<Buffer> {
  ...
  const sticky = await page.evaluate(measureStickyElements);
  ...
  return stitchStrips({ strips, documentHeight, viewportWidth: viewport.width, dpr: DEVICE_SCALE_FACTOR });
}
