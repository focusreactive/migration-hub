...
export function isolateMarked(args: IsolateArgs): boolean {
  const marked = (Array.prototype.slice.call(document.querySelectorAll(`[${args.markAttribute}]`)) as MigElement[])[0];
  if (marked === undefined) return false;

  for (let node = marked.parentElement; node !== null; node = node.parentElement) {
