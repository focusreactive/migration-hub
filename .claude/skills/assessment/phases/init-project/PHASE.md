...
Every state change runs the script — never write `.assessment/*` by hand.
...
`<workspace.path from assessment.config.json>/<project-name>`. No network I/O
happens here.

- **`status: "created"`** — a fresh project. `.assessment/`, `run-config.json`
...
- **`status: "existing"`** — a project for this same URL already has a
  ...
  `<projectPath>/.assessment/manifest.json` and resume from the first step in
...
Read `<projectPath>/.assessment/manifest.json`: it has `sourceUrl`,
`toolVersion` and `steps["init-project"].status == "done"`.
`<projectPath>/.assessment/run-config.json` holds `{ sourceUrl, projectName }`.
