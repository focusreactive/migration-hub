...
import { escapeAttr } from "../../../../../src/scripts/report/html/utils/escape.ts";
...
describe("term", () => {
  it("renders the design's tooltip span", () => {
    expect(term("uniqueLayoutPage", "unique layout pages")).toBe(
      `<span class="term" tabindex="0" data-def="${escapeAttr(GLOSSARY_TERMS.uniqueLayoutPage)}">unique layout pages</span>`,
    );
  });
