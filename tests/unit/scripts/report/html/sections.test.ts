...
import { modalSection } from "../../../../../src/scripts/report/html/sections/modal.ts";
...
describe("modalSection", () => {
  it("renders every hook the page script addresses", () => {
    const html = modalSection();
...
describe("PAGE_SCRIPT", () => {
  it("carries the three substitution placeholders and no others", () => {
    ...
    expect(PAGE_SCRIPT).toContain("__TYPES__");
  });

  it("reads screenshots from the SHOTS map rather than a file path", () => {
    expect(PAGE_SCRIPT).toContain("SHOTS[");
    expect(PAGE_SCRIPT).not.toContain("'../assets/'");
  });

  it("fills in every data-shot image on load", () => {
    expect(PAGE_SCRIPT).toContain("img[data-shot]");
  });

  it("derives the page-link tooltip hostname from SITE rather than a hardcoded design-mock domain", () => {
    expect(PAGE_SCRIPT).toContain("SITE_HOST");
    expect(PAGE_SCRIPT).not.toContain("nova-x.webflow.io");
  });

  it("keeps the filter, disclosure and progress behaviour", () => {
    expect(PAGE_SCRIPT).toContain("secFilters");
    expect(PAGE_SCRIPT).toContain("pageToggle");
    expect(PAGE_SCRIPT).toContain("scrollProgress");
  });
